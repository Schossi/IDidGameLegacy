...
var IDIDGAME_Water = (function (pub, undefined) {
    ...
    pub.Drawable.draw = function () {
        ...
        if (this.commitSorted == false) {
            this.commitSorted = true;
            if (this.drawParent) {
        ...
        if (this.animationQuantity) {
            ...
            if (this.animationCounter >= this.animationInterval) {
                ...
                if (this.animationFrame >= this.animationQuantity) {
                    this.animationFrame = 0;
                    this.spriteX -= this.animationX * (this.animationQuantity);
                    this.spriteY -= this.animationY * (this.animationQuantity);
                } else {
    ...
    pub.DrawableBoxCircle = RSTools.createObject(pub.DrawableCircle);
    pub.DrawableBoxCircle.initBoxCircle = function () {
        var that = this;

        this.fixDef = new Box2D.Dynamics.b2FixtureDef;
        this.bodyDef = new Box2D.Dynamics.b2BodyDef;

        this.bodyDef.type =this.bodyDevType ||  Box2D.Dynamics.b2Body.b2_dynamicBody;
        this.fixDef.shape = new Box2D.Collision.Shapes.b2CircleShape;
        this.fixDef.shape.SetRadius(this.radius/pub.boxscale);
        this.fixDef.friction = this.friction || 0;
        this.fixDef.density = this.density || 1;
        this.bodyDef.position.x = this.centerX / pub.boxscale;
        this.bodyDef.position.y = this.centerY / pub.boxscale;
        this.retVal = this.gameContext.BoxWorld.CreateBody(this.bodyDef).CreateFixture(this.fixDef);

        this.body = this.retVal.GetBody();

        this.gameContext.toUpdate.push(this);
    };
    pub.DrawableBoxCircle.update = function () {

        var pos = this.body.GetPosition();

        //this.rotation = this.body.GetAngle();

        this.centerX = pos.x * pub.boxscale;
        this.centerY = pos.y * pub.boxscale;
    };
    ...
    pub.DrawableBoxRect = RSTools.createObject(pub.DrawableRect);
    pub.DrawableBoxRect.initBoxRect = function () {
        var that = this;

        this.fixDef = new Box2D.Dynamics.b2FixtureDef;
        this.bodyDef = new Box2D.Dynamics.b2BodyDef;
        if (this.bodyDevType === undefined) {
            this.bodyDef.type = Box2D.Dynamics.b2Body.b2_dynamicBody;
        } else {
            this.bodyDef.type = this.bodyDevType ;
        }
        
        this.fixDef.shape = new Box2D.Collision.Shapes.b2PolygonShape;
        this.fixDef.shape.SetAsBox(this.width / (pub.boxscale * 2), this.height / (pub.boxscale*2));
        this.fixDef.friction = this.friction || 0;
        this.fixDef.density =this.density || 1;
        this.bodyDef.position.x = this.centerX / pub.boxscale;
        this.bodyDef.position.y = this.centerY / pub.boxscale;
    ...
    pub.DrawableBoxRect.update = function () {
        var pos;
    ...
    pub.DrawableText.addToContext = function () {
        this.gameContext.addToDraw(this);
        this.gameContext.toCommit.push(this);
    };
    pub.DrawableText.commitPosition = function (_changeBit) {
        if (this.changeBit === _changeBit) {
            return;
        }
    ...
    pub.DrawableText.draw = function () {
        ...
        if (this.strokeStyle) {
            context.strokeStyle = this.strokeStyle;
            context.strokeText(this.text, drawX, this.posY);
        }
    ...
    pub.DrawableText.remove = function () {
        if (this.zIndex >= 10000) {
            RSTools.removeFromArray(this.gameContext.toDrawUI, this);
        } else {
            RSTools.removeFromArray(this.gameContext.toDraw, this);
        }
    };
    ...
    pub.RepeatingDrawable.init = function (_gameContext, _posX, _posY, _spriteX, _spriteY, _width, _height, _amountX, _amountY, _zIndex, _offsetX, _offsetY, _drawHeight, _drawWidth, _static) {
        ...
        for (x = 0; x < this.amountX; x++) {
            for (y = 0; y < this.amountY; y++) {
                if (this.randTile) {
                    while (randAddend === lastAddend) {
                        randAddend = RSTools.getRandomInt(this.randTile) * this.height;
                    }

                    lastAddend = randAddend;
                }
            
                draw = RSTools.createObject(pub.Drawable);
                draw.initDrawable(this.gameContext, this.posX + this.offsetX * x, this.posY + this.offsetY * y, this.spriteX, this.spriteY+randAddend, this.width, this.height, this.zIndex);
